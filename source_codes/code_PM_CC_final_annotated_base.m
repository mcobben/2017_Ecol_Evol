%% IN THIS VERSION WE USE SWITCHES TO SWITCH BETWEEN DIFFERENT RUN- AND INITIALISATION SETTINGS
...
basefolder='P:\Marleen\Migration\climate_change\matlab\output\rep_runs\'; % where the different scenario folders are stored
scens=dir([basefolder 'scen_*']); % reads all the scenario folders

for scen=20:length(scens)
    ...
    for run=1:100
        ...
        % genetic diversity
        if switchG==1
            ind(:,1)= Tfit; % initialisation with zero genetic diversity
        elseif switchG==2
        ...
        if year < year_change % 1 301 year_max
