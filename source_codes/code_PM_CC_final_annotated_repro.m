%% IN THIS VERSION WE LOWER MIGRANT REPRODUCTION UNDER CLIMATE CHANGE

clear all; 
close all; 
clc;
dbstop if error;
location='P:\Marleen\Migration\climate_change\matlab'; % where the .m file is stored

%% location and scenarios input

basefolder='P:\Marleen\Migration\climate_change\matlab\output\rep_runs\rep_runs_repr\'; % where the different scenario folders are stored
...
for scen=1:length(scens)
    ...
    for run=1:100
        ...
        elseif switchL==2
            ind(:,3)=ceil(rand(N_ind,1)*y_ini); % initialisation with random y location in native area at min y side / migrants only
        elseif switchL==3
            ind(:,3)=101-ceil(rand(N_ind,1)*y_ini); % initialisation with random y location in native area at max y side / residents only
        end
        ...
        if year < year_change 
            ...
            surv_prob = std_surv *randn + (ind(:,3)/100)+delta_surv; % after a certain period the winter survival of the residents changes abruptly with delta_surv 
        end
        ...
        if ismember(year,yrs_output)==1
            ind=ind_w;
            resultfile=[datafolder scens(scen,1).name '_run' num2str(run) '_yr' num2str(year)];
            save(resultfile, 'ind','switchL','switchG','delta_surv','year_tot','year_change','yrs_output');
        end
        ...
        if year < year_change 
            ind_r(:,5)=floor(ind_r(:,5).*(rand(N_ind_d,1)*7));
        else
            ind_r(:,5)=floor((ind_r(:,5).*(rand(N_ind_d,1)*7))-0.2*ind_r(:,4)); %random nr of offspring [0..6], corrected for migrants -0.2
        end
