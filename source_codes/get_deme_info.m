function [deme, deme_t, deme_t_std, deme_m, deme_r]=get_deme_info(y_max,x_max,ind,deme_count_base,method)
...
switch method
    case 'complete'
        deme_t=zeros(y_max,x_max); deme_t_std=zeros(y_max,x_max); deme_m=zeros(y_max,x_max); deme_r=zeros(y_max,x_max);
        for dm=1:length(index_c)-1
            ...
            deme_t_std(row,column)=std(ind(index_c(dm):index_c(dm+1)-1,1)); 
        ...
        deme_t_std(end)=std(ind(index_c(dm+1):end,1));
    ...
    case 'deme_m_only'
        deme_m=zeros(y_max,x_max); deme_t=zeros(y_max,x_max);deme_t_std=zeros(y_max,x_max);deme_r=zeros(y_max,x_max);
        for dm=1:length(index_c)-1
            row=mean(ind(index_c(dm):index_c(dm+1)-1,3));
            column=mean(ind(index_c(dm):index_c(dm+1)-1,2));
            deme_m(row,column)=sum(ind(index_c(dm):index_c(dm+1)-1,4));  
